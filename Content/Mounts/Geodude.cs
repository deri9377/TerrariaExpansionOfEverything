...
namespace TerrariaExpansionOfEverything.Content.Mounts
{
	...
	public class Geodude : ModMount
	{
		...
		public override void SetStaticDefaults() {
			...
			MountData.buff = ModContent.BuffType<MountBuff>();
		...
		public override bool Draw(List<DrawData> playerDrawData, int drawType, Player drawPlayer, ref Texture2D texture, ref Texture2D glowTexture, ref Vector2 drawPosition, ref Rectangle frame, ref Color drawColor, ref Color glowColor, ref float rotation, ref SpriteEffects spriteEffects, ref Vector2 drawOrigin, ref float drawScale, float shadow) {
			...
			if (drawType == 0) {
				...
				Texture2D geodudeTexture = Mod.Assets.Request<Texture2D>("Content/Mounts/Geodude_front").Value;

				var position = drawPosition + new Vector2(-30, geodudeTexture.Height + 30);
				var srcRect = new Rectangle(0, 0, geodudeTexture.Width, geodudeTexture.Height);
				float drawRotation = rotation;
				var origin = new Vector2(14 + drawPlayer.direction * 7, 42);

				playerDrawData.Add(new DrawData(geodudeTexture, position, srcRect, drawColor, drawRotation, origin, drawScale, spriteEffects ^ SpriteEffects.FlipHorizontally, 0));
			}
